...
def prompt(body: PromptIn):
    try:
        ...
        # 2) Retrieve more candidates (so we can keep unique talk_id)
        res = index.query(
            vector=q_vec,
        ...
        # 3) Keep at most 1 chunk per talk_id (distinct talks)
        context = []
        ctx_text_blocks = []
        seen_talk_ids = set()
        ...
        # 5) Ask chat model (no temperature for gpt-5-mini)
